...
lsCovers <- data.table(nm = dir('output/data-prep/cropped-rasters/RMNP', '.tif$'))[, 
  nm := gsub(".tif|100m", "", nm)]$nm[-1][-3]
lsPaths <- dir('output/data-prep/cropped-rasters/RMNP', '.tif$', full.names = TRUE)[-1][-3]
...
winterElkRSF.rstr <- (lsRasters[[1]] * winElk.b[1] + lsRasters[[2]] * winElk.b[2] +
                        lsRasters[[3]] * winElk.b[3] + resample(lsRasters[[4]], lsRasters[[3]]) * winElk.b[4] + 
...
springElkRSF <- glm(reformulate(lsCovers, response = 'observed'),  #### Dist not logged yet
