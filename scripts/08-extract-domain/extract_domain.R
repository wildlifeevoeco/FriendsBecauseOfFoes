...
rastlist <- list.files(path = "output/DOMAINS", 
                       pattern =".tif$", full.names=TRUE)

allrasters.NL <- lapply(rastlist[c(1:18)], raster)
allrasters.RMNP <- lapply(rastlist[c(19:26)], raster)
...
save.image("ready_for_social_new.Rdata")

rm(list=setdiff(ls(), c("caribouExtract", "elkExtract")))

save.image("for_step4_ewc_new.Rdata")
