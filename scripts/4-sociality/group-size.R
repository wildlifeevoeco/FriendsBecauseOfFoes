message('=== Nearest Neighbour Analysis - GROUP SIZE ===')
...
out <- edges[DT[, .SD, .SDcols = cols], on = .(timegroup, ID = id)]



# 
# DT[edges, on = .(timegroup, id = ID)]
# 
...
saveRDS(out, paste0('output/4-sociality/', species, 'NNA-group-ALR.Rds'))
