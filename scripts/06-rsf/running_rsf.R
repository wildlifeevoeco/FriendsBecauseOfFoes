...
elk.win.st1 <- glm(case ~ log(bg + min.log_RMNP) + log(cf + min.log_RMNP) + log(mh + min.log_RMNP) + log(mf + min.log_RMNP) + log(od + min.log_RMNP) + lf + wt + log(ru + min.rlog_RMNP),  family = "binomial", data=elk2h_rsf_win_st1)
elk.win.st2 <- glm(case ~ log(bg + min.log_RMNP) + log(cf + min.log_RMNP) + log(mh + min.log_RMNP) + log(mf + min.log_RMNP) + log(od + min.log_RMNP) + lf + wt + log(ru + min.rlog_RMNP),  family = "binomial", data=elk2h_rsf_win_st2)

wolf.win.st1 <- glm(case ~ log(bg + min.log_RMNP) + log(cf + min.log_RMNP) + log(mh + min.log_RMNP) + log(mf + min.log_RMNP) + log(od + min.log_RMNP) + lf + wt + log(ru + min.rlog_RMNP),  family = "binomial", data=wolf1h_rsf_win_st1)
wolf.win.st2 <- glm(case ~ log(bg + min.log_RMNP) + log(cf + min.log_RMNP) + log(mh + min.log_RMNP) + log(mf + min.log_RMNP) + log(od + min.log_RMNP) + lf + wt + log(ru + min.rlog_RMNP),  family = "binomial", data=wolf1h_rsf_win_st2)
...
coyote4h_rsf<-coyote4h_rsf[,c(1:39,42:44,40,45,41)]  ### not sure where they got unordered but this fixes them to the same order
...
coyote8h_rsf<-coyote8h_rsf[,c(1:39,42:44,40,45,41)] ### not sure where they got unordered but this fixes them to the same order

coyote8h_rsf_win_st1<-subset(coyote8h_rsf, coyote8h_rsf$season=="winter" & (coyote8h_rsf$STATE2==1| is.na(coyote8h_rsf$STATE2)))
coyote8h_rsf_win_st2<-subset(coyote8h_rsf, coyote8h_rsf$season=="winter" & (coyote8h_rsf$STATE2==2| is.na(coyote8h_rsf$STATE2)))
...
cor.coyote4h.spr.st1<-round(cor(coyote4h_rsf_spr_st1[,c(39:45)]),2)
cor.coyote4h.spr.st2<-round(cor(coyote4h_rsf_spr_st2[,c(39:45)]),2)

cor.coyote8h.win.st1<-round(cor(coyote8h_rsf_win_st1[,c(39:45)]),2)
cor.coyote8h.win.st2<-round(cor(coyote8h_rsf_win_st2[,c(39:45)]),2)
...
caribou.spr.st1 <- glm(case ~ log(f + min.log_NL) + log(l + min.log_NL) + log(r + min.log_NL) + log(s + min.log_NL) + lf + w + log(ru + min.rlog_NL),  family = "binomial", data=caribou2h_rsf_spr_st1)
caribou.spr.st2 <- glm(case ~ log(f + min.log_NL) + log(l + min.log_NL) + log(r + min.log_NL) + log(s + min.log_NL) + lf + w + log(ru + min.rlog_NL),  family = "binomial", data=caribou2h_rsf_spr_st2)
caribou.win.st1 <- glm(case ~ log(f + min.log_NL) + log(l + min.log_NL) + log(r + min.log_NL) + log(s + min.log_NL) + lf + w + log(ru + min.rlog_NL),  family = "binomial", data=caribou2h_rsf_win_st1)
caribou.win.st2 <- glm(case ~ log(f + min.log_NL) + log(l + min.log_NL) + log(r + min.log_NL) + log(s + min.log_NL) + lf + w + log(ru + min.rlog_NL),  family = "binomial", data=caribou2h_rsf_win_st2)

bear.spr.st1 <- glm(case ~ log(f + min.log_NL) + log(l + min.log_NL) + log(r + min.log_NL) + log(s + min.log_NL) + lf + w + log(ru + min.rlog_NL),  family = "binomial", data=bear2h_rsf_spr_st1)
bear.spr.st2 <- glm(case ~ log(f + min.log_NL) + log(l + min.log_NL) + log(r + min.log_NL) + log(s + min.log_NL) + lf + w + log(ru + min.rlog_NL),  family = "binomial", data=bear2h_rsf_spr_st2)

coyote4.spr.st1 <- glm(case ~ log(f + min.log_NL) + log(l + min.log_NL) + log(r + min.log_NL) + log(s + min.log_NL) + lf + w + log(ru + min.rlog_NL),  family = "binomial", data=coyote4h_rsf_spr_st1)
coyote4.spr.st2 <- glm(case ~ log(f + min.log_NL) + log(l + min.log_NL) + log(r + min.log_NL) + log(s + min.log_NL) + lf + w + log(ru + min.rlog_NL),  family = "binomial", data=coyote4h_rsf_spr_st2)

coyote.win.st1 <- glm(case ~ log(f + min.log_NL) + log(l + min.log_NL) + log(r + min.log_NL) + log(s + min.log_NL) + lf + w + log(ru + min.rlog_NL),  family = "binomial", data=coyote8h_rsf_win_st1)
coyote.win.st2 <- glm(case ~ log(f + min.log_NL) + log(l + min.log_NL) + log(r + min.log_NL) + log(s + min.log_NL) + lf + w + log(ru + min.rlog_NL),  family = "binomial", data=coyote8h_rsf_win_st2)
...
write.csv(res.RMNP, "OUTPUT_NEW/rsf_results/log_rsf_res_RMNP.csv")
write.csv(res.NL, "OUTPUT_NEW/rsf_results/log_rsf_res_NL.csv")


####  RERUN and RESAVE TO REMOVE CHAFF  ######

save.image("OUTPUT_NEW/rsf_results/log_RSF_MODELS.Rdata")
