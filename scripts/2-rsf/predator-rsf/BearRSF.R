...
nlBounds <- spTransform('input/etc/NL-Bounds/NL-Bounds.shp',
                        CRSobj = utm)
...
BearPoints<-SpatialPoints(data.frame(bears$EASTING,bears$NORTHING),proj4string = CRS(utm))
...
BearRandPoints<-SpatialPoints(data.frame(regPts[,1],regPts[,2]),proj4string = CRS(utm))
...
BearUsedData<-data.frame(1,BearAntUsed,BearBroUsed,BearConUsed,BearLicUsed,BearMixUsed,BearRocUsed,
                         BearScrUsed,BearWatUsed,BearWetUsed,BearRugUsed,BearWaDUsed,BearLinUsed,bears$EASTING,bears$NORTHING,bears$season)

colnames(BearUsedData)<-CNames

BearAvailData<-data.frame(0,BearAntAvail,BearBroAvail,BearConAvail,BearLicAvail,BearMixAvail,BearRocAvail,BearScrAvail,
                          BearWatAvail,BearWetAvail,BearRugAvail,BearWaDAvail,BearLinAvail,regPts[,1],regPts[,2],"Avail")
...
RSFbearsSum<-glm(use~Ant+Bro+Con+Lic+Mix+Roc+Scr+WaD+Lin+Rug,data=BearSummer, family='binomial')


summary(RSFbearsSum)
rsquared(RSFbearsSum)

SumRSF<-exp(coef(RSFbearsSum)[1]+AntR*coef(RSFbearsSum)[2]+BroR*coef(RSFbearsSum)[3]+ConR*coef(RSFbearsSum)[4]+LicR*coef(RSFbearsSum)[5]+
              MixR*coef(RSFbearsSum)[6]+RocR*coef(RSFbearsSum)[7]+ScrR*coef(RSFbearsSum)[8]+WaDR*coef(RSFbearsSum)[9]+
